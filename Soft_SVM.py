    ...
    def train(self, X, y):
        ...
        for _ in range(self.epochs):
            for idx, V in enumerate(X):
                ...
                if y[idx] * (np.dot(self.w,np.transpose(V))) + slack < 1:
                    
                    
                    loss_gradient = np.multiply(y[idx],np.transpose(V)) + np.transpose(V)

                    

                    self.w = self.w + np.multiply(self.learning_rate, loss_gradient)
                    
                    #                     y[1] * V[(1 + p) x 1]


                    # Soft SVM Modification

                    # slack = self.w * V - y[idx]

                    # hinge_loss = 1 - y[idx] * self.w * V 

                    # hinge_loss = 1 - y[idx] * self.w * V  + self.w * V - y[idx] 
